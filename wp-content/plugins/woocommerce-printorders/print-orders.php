...
class WooCommerce_Ext_PrintOrders {
	...
	public function ajax() {
		...
		if (isset($_REQUEST['subaction']) && 'print-internal' == $_REQUEST['subaction']) {
			...
			$output = $this->getdocument_internal($order, $order_id, $order->get_items());
			...
			if (class_exists('WC_Log_Handler_File')) {
				...
				echo "Class WC_Log_Handler_File not found.";
...
// 		$paper_sizes = $this->get_paper_sizes();
	...
	public function woocommerce_checkout_order_processed($order_id_or_order) {
		...
		$this->log("Order payment method: ".$payment_method.". Decision on whether to print now or wait for payment confirm: ".($print_order_now ? 'Now' : 'Wait'));
	...
	public function woocommerce_process_order($order_id, $printer_id = false, $type_key = false) {
		...
		*/
		...
		if (false !== $type_key) {
			$destinations = array($type_key);
		} else {
			$destinations = $this->get_destinations_from_options($opts);
		}
